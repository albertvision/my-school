...
public interface MarkRepository extends ListCrudRepository<Mark, Long> {
    ...
    @Query("""
            ...
            WHERE m.studyPeriod.id = :studyPeriodId
            AND m.schoolClass.id = :schoolClassId
            AND m.teacher.id = :teacherId
            ORDER BY m.id ASC
            """)
    List<MarkAndStudentNames> findByStudyPeriodIdAndSchoolClassIdAndTeacherIdWithStudentNamesOrderByIdAsc(@Param("studyPeriodId") Long studyPeriodId, @Param("schoolClassId") Long schoolClassId, @Param("teacherId") Long teacherId);
